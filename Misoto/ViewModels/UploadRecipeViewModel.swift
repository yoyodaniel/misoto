...
class UploadRecipeViewModel: ObservableObject {
    ...
    /// Use AI to either generate instructions (when empty) or edit existing ones for spelling/grammar
    func editInstructionsWithAI() async {
        let validInstructions = instructions.filter { !$0.text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        let hasExistingInstructions = !validInstructions.isEmpty
        
        // If no instructions, generate from title + ingredients
        if !hasExistingInstructions {
            ...
            do {
                ...
                let generatedSteps = try await OpenAIService.generateInstructions(
                    title: title,
                    ingredients: allIngredients
                )
                
                // Replace instructions with generated steps
                instructions = generatedSteps.map { text in
                    InstructionItem(text: text)
                }
                print("✅ Instructions generated by AI: \(generatedSteps.count) steps")
            } catch {
        ...
        // Otherwise, edit existing instructions for spelling/grammar
        isEditingInstructions = true
        errorMessage = nil
        
        do {
            let instructionTexts = instructions.map { $0.text }
            let editedTexts = try await OpenAIService.editInstructions(instructionTexts)
            
            // Update instructions with edited text (preserve images/videos)
            for i in 0..<min(instructions.count, editedTexts.count) {
                instructions[i].text = editedTexts[i]
            }
            print("✅ Instructions edited by AI successfully")
        } catch {
            errorMessage = LocalizedString("Failed to edit instructions: \(error.localizedDescription)", comment: "AI instruction edit error")
        }
